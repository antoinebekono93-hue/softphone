...
import { getConfiguredTelnyxApiKey, getConfiguredTelnyxClient, getConfiguredTelnyxPublicKey } from "@/lib/telnyx";
...
export async function POST(req: Request) {
  ...
  try {
          ...
          try {
            ...
            if (employee && employee.openaiAssistantId) {
              ...
              if (isVoiceNote && audioUrl) {
                ...
                try {
                  const telnyxApiKey = await getConfiguredTelnyxApiKey();
                  const mediaRes = await fetch(audioUrl, {
                    headers: { Authorization: `Bearer ${telnyxApiKey}` },
                  });
                  if (!mediaRes.ok) throw new Error(`Téléchargement média Telnyx refusé (${mediaRes.status}).`);
    ...
    } else if (['message.status.updated', 'message.sent', 'message.finalized'].includes(eventType)) {
      const payloadInfo = eventData.payload;
      const telnyxMessageId = payloadInfo.id;
      const status = payloadInfo.status || payloadInfo.to?.[0]?.status || (eventType === 'message.sent' ? 'sent' : null);

      if (typeof telnyxMessageId !== 'string' || typeof status !== 'string') {
        console.warn('[WhatsApp webhook] Événement de statut incomplet', { eventType });
      ...
      if (message) {
        ...
        try {
          ...
          await getPusherServer()?.trigger(
            ...
            'message-status', 
            { messageId: message.id, status: normalizedStatus, contactId: message.contactId }
          );
        } catch (e) {
        ...
        if (campaignRecipient) {
          const newStatus = normalizedStatus; // SENT, DELIVERED, READ, DELIVERY_FAILED…
