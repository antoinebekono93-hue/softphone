...
function withBodyExamples(components: any[]) {
  return components.map(component => {
    if (String(component?.type).toUpperCase() !== 'BODY' || typeof component?.text !== 'string') return component;
    const variables = [...component.text.matchAll(/\{\{(\d+)\}\}/g)]
      .map(match => Number(match[1]))
      .filter(Number.isFinite);
    if (!variables.length || component.example?.body_text) return component;
    const count = Math.max(...variables);
    return {
      ...component,
      example: {
        ...(component.example || {}),
        body_text: [Array.from({ length: count }, (_, index) => `Exemple ${index + 1}`)],
      },
    };
  });
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const response = await telnyx.whatsapp.templates.create({
      ...
      components: withBodyExamples(input.components),
    });
