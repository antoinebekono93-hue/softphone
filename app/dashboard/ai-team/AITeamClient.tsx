...
export default function AITeamClient({ initialEmployees, phoneNumbers, whatsappAccounts }: any) {
  ...
  // Handle OAuth success/error alerts
  useEffect(() => {
    if (searchParams) {
      ...
      if (success === 'social_connected') {
        toast.success("Compte social connecté avec succès !");
        // Clear params
        window.history.replaceState(null, '', '/dashboard/ai-team');
      } else if (error) {
  ...
  const handleConnectSocial = async (provider: string, employeeId: string) => {
    try {
      // Pour WhatsApp, l'Embedded Signup de Meta utilise le SDK JS (FB.login).
      // Dans cette version, nous redirigeons vers notre endpoint OAuth unifié.
      // Quand l'App Meta sera configurée pour Embedded Signup, on pourra 
      // lancer FB.login() ici au lieu de rediriger.
      
      const toastId = toast.loading(`Redirection vers ${provider}...`);
      window.location.href = `/api/auth/meta?employeeId=${employeeId}&provider=${provider}`;
      // On masque le toast après 2s car la page va changer
  ...
  const handleToggleMetaSetting = async (employeeId: string, setting: 'replyToCommentsPublicly' | 'replyToCommentsPrivately', value: boolean) => {
    try {
      // Optimistic update
      const updatedEmployees = employees.map((emp: any) => 
        emp.id === employeeId ? { ...emp, [setting]: value } : emp
      );
      setEmployees(updatedEmployees);

      const res = await fetch("/api/ai-employees/meta-settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ employeeId, [setting]: value })
      });
      if (!res.ok) throw new Error("Failed to update setting");
      toast.success("Paramètre mis à jour");
    } catch (error) {
      toast.error("Erreur lors de la mise à jour");
      // Revert in real app, but for now it's ok
    }
  };
                ...
                <div>
                  ...
                  {emp.roleType === 'FACEBOOK_MANAGER' ? (
                    <>
                      ...
                      <div className="mt-4 p-3 bg-[var(--bg-base)] rounded-xl border border-[var(--border-subtle)] space-y-3">
                        <span className="text-xs font-semibold text-[var(--text-secondary)] uppercase">Gestion des Commentaires</span>
                        <label className="flex items-center justify-between gap-2 text-sm text-[var(--text-primary)] cursor-pointer hover:bg-[var(--bg-surface-hover)] p-1 -mx-1 rounded transition-colors">
                          <span className="flex-1">Répondre Publiquement</span>
                          <input type="checkbox" checked={emp.replyToCommentsPublicly ?? true} onChange={(e) => handleToggleMetaSetting(emp.id, 'replyToCommentsPublicly', e.target.checked)} className="w-4 h-4 rounded text-[#1877F2] focus:ring-[#1877F2] bg-[var(--bg-base)] border-[var(--border-subtle)]" />
                        </label>
                        <label className="flex items-center justify-between gap-2 text-sm text-[var(--text-primary)] cursor-pointer hover:bg-[var(--bg-surface-hover)] p-1 -mx-1 rounded transition-colors">
                          <span className="flex-1">Répondre en Privé (DM)</span>
                          <input type="checkbox" checked={emp.replyToCommentsPrivately ?? false} onChange={(e) => handleToggleMetaSetting(emp.id, 'replyToCommentsPrivately', e.target.checked)} className="w-4 h-4 rounded text-[#1877F2] focus:ring-[#1877F2] bg-[var(--bg-base)] border-[var(--border-subtle)]" />
                        </label>
                      </div>
                    </>
