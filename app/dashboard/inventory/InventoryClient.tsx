...
export default function InventoryClient({ products, store, carts, orgId }: { products: any[], store: any, carts: any[], orgId: string }) {
  ...
  return (
    <div className="space-y-6">
      <Tabs
        value={activeTab}
        onValueChange={(v) => setActiveTab(v as "CATALOG" | "CARTS" | "SETTINGS")}
        variant="underline"
        aria-label="Sections inventaire"
        tabs={[
          { id: "CATALOG", label: (<span className="flex items-center gap-2"><Package className="w-4 h-4" /> Catalogue ({products.length})</span>) },
          { id: "CARTS", label: (<span className="flex items-center gap-2"><ShoppingCart className="w-4 h-4" /> Paniers Abandonnés</span>) },
          { id: "SETTINGS", label: (<span className="flex items-center gap-2"><Settings2 className="w-4 h-4" /> Shopify Settings</span>) },
        ]}
      />
