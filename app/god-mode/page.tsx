    ...
    <div className="w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="glass-panel border-none rounded-2xl p-6 flex flex-col relative overflow-hidden">
           ...
           <span className="text-4xl font-bold text-[var(--text-primary)]">{usersCount}</span>
         ...
         <div className="space-y-4">
            <div className="flex items-center justify-between p-4 border border-[var(--border-subtle)] rounded-xl bg-[var(--bg-surface-hover)]">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     <p className="font-medium">Database (Supabase / Postgres)</p>
            ...
            <div className="flex items-center justify-between p-4 border border-[var(--border-subtle)] rounded-xl bg-[var(--bg-surface-hover)]">
               <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${telnyxReady ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>
                     <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                  </div>
                  <div>
                     <p className="font-medium">Telnyx SIP & API Gateway</p>
                     <p className="text-xs text-[var(--text-secondary)]">
                       {telnyxReady ? "API Keys present in environment" : "Missing API Keys in .env"}
                     </p>
                  </div>
               </div>
               <span className={`px-3 py-1 text-xs font-medium rounded-full ${telnyxReady ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}`}>
                 {telnyxReady ? 'Connected' : 'Pending Auth'}
               </span>
            </div>
         </div>
      </div>
