"use server";
/* eslint-disable @typescript-eslint/no-explicit-any -- Telnyx SDK resources expose heterogeneous paginated payloads. */
...
export async function uploadWhatsAppProfilePhoto(phoneNumberRaw: string, formData: FormData) {
  await requireSuperAdmin();
  const phoneNumber = canonicalizePhoneNumber(phoneNumberRaw);
  ...
  const telnyx = await getConfiguredTelnyxClient();
  try {
    return (await telnyx.whatsapp.phoneNumbers.profile.photo.upload(phoneNumber, { file })).data || null;
  } catch (error) { throw new Error(message(error)); }
}

export async function deleteWhatsAppProfilePhoto(phoneNumberRaw: string) {
  await requireSuperAdmin();
  const phoneNumber = canonicalizePhoneNumber(phoneNumberRaw);
  if (!phoneNumber) throw new Error('Numéro E.164 invalide.');
  const telnyx = await getConfiguredTelnyxClient();
  try { await telnyx.whatsapp.phoneNumbers.profile.photo.delete(phoneNumber); }
  catch (error) { throw new Error(message(error)); }
}
