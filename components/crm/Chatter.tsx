  ...
  return (
    <div className="h-full flex flex-col bg-[#111118]">
        ...
        <div className="p-4">
          ...
          <div className="flex justify-between items-center mt-3">
            ...
            <button 
              ...
              disabled={isSubmitting}
              className={`px-4 py-1.5 rounded-lg text-sm font-bold text-white transition-colors ${
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {isLoading ? (
          <div className="text-center text-gray-500 text-sm mt-4">Chargement de l'historique...</div>
        ) : feed.length === 0 ? (
          <div className="text-center text-gray-500 text-sm mt-4">Aucune activité récente.</div>
        ) : (
          feed.map((item) => (
            <div key={item.id} className="flex gap-3">
              <div className="mt-1">
                ...
                {item.type === "whatsapp_out" && <div className="w-8 h-8 rounded-full bg-emerald-900/50 flex items-center justify-center text-emerald-400"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 2L11 13"></path><path d="M22 2l-7 20-4-9-9-4 20-7z"></path></svg></div>}
              ...
              <div className={`flex-1 bg-white/[0.03] border border-white/5 rounded-2xl p-3 ${item.type.includes('whatsapp') ? "border-emerald-500/20 bg-emerald-900/10" : ""}`}>
        ...
        {!isLoading && feed.length > 0 && (
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Fin de l'historique</span>
            <div className="flex-1 h-px bg-white/10"></div>
          </div>
        )}
