    ...
    <div className="flex gap-4 h-full overflow-x-auto pb-4">
      {STAGES.map(stage => {
        ...
        return (
          <div 
            key={stage.id} 
            className="flex-shrink-0 w-[300px] flex flex-col bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl overflow-hidden"
            onDragOver={handleDragOver}
            onDrop={(e) => handleDrop(e, stage.id)}
          >
            {/* Column Header */}
            <div className="p-3 border-b border-[var(--border-subtle)] bg-[var(--bg-surface-solid)] flex items-center justify-between cursor-pointer hover:bg-[var(--bg-surface-hover)]">
              <h3 className="font-bold text-[var(--text-primary)]">{stage.name}</h3>
              <div className="text-xs font-bold bg-[var(--bg-elevated)] text-[var(--text-secondary)] px-2 py-1 rounded-md">
...
className="bg-[var(--bg-surface-solid)] border border-[var(--border-subtle)] rounded-lg p-3 cursor-pointer hover:border-[var(--border-glow)] hover:shadow-md transition-all group"
                  >
                  <div className="flex justify-between items-start mb-2">
                    <div className="font-bold text-[var(--text-primary)] text-sm group-hover:text-[var(--brand)] transition-colors">{opp.name}</div>
                  </div>
                  
                  <div className="text-xs text-[var(--text-secondary)] mb-3 font-medium">{opp.company}</div>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-2 border-t border-[var(--border-subtle)]">
                    ...
                    <div className="flex gap-0.5 text-[var(--text-muted)]">
                      {[1, 2, 3].map(star => (
              ...
              {/* Quick Add Button */}
              <button onClick={() => onAddOpportunity(stage.id)} className="w-full py-2 flex items-center justify-center gap-1 text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface-hover)] text-sm font-bold border border-transparent hover:border-[var(--border-subtle)] rounded-lg transition-all">
