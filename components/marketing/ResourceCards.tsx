  ...
  return (
    <div className="grid md:grid-cols-3 gap-6">
      {items.map((item) => (
        ...
        >
          <div className="text-xs font-bold uppercase tracking-widest text-cyan-400">{item.label}</div>
          <div className="text-lg font-bold text-[var(--text-primary)] group-hover:text-cyan-400 transition-colors">
            {item.title}
          </div>
          <p className="text-sm text-[var(--text-secondary)] font-medium leading-relaxed">{item.description}</p>
          <div className="mt-auto text-sm font-bold text-cyan-400 flex items-center gap-1">
            En savoir plus
